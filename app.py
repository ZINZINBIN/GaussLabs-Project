...
def build_tab_3(table_columns = table_columns, table_data = table_data, bar_x = bar_x, bar_y = bar_y, bar_histo_trace = bar_histo_trace):
    return html.Div(
        id = "Data-analysis-container",
        children = [
            html.Div(
                id="Data-analysis-table-chart",
                children=[
                    generate_section_banner("Data Table"),
                    dash_table.DataTable(
                        ...
                        style_cell_conditional=[
                            {'if': {'column_id': 'Batch'},
                             'width': '50%'},
                            {'if': {'column_id': table_columns[1]['name']},
                             'width': '50%'},
                        ],
                        css = [{
                        ...
                        figure=go.Figure(
                            {
                                ...
                                "layout": {
                                    ...
                                    "legend" : {"font": {"color": "#4e4e4e"}, "orientation": "h", "x": 0, "y": 1.1},
                                    "font" :{"color": "#4e4e4e"},
                                    "xaxis": dict(
                                        showline=True, showgrid=False, zeroline=True, title=bar_histo_trace["name"]
                                    ),
                                    "yaxis": dict(
                                        showgrid=False, showline=True, zeroline=True, title = "Frequency"
                                    ),
                                    "autosize": True,
                                },
            ...
            "data": [
                {
                    ...
                    "values": [],
                    "type": "pie",
                    "marker": {"line": {"color": "#4e4e4e", "width": 1}},
                    "hoverinfo": "label",
                    "textinfo": "label",
                }
            ],
    ...
    if ctx.triggered:
        ...
        if prop_type == "n_clicks":
            ...
            if curr_id == prop_id:
                ...
                # data-table
                batch_y = state_dict["Batch"]["data"].tolist()
                curr_y = state_dict[curr_id]["data"].tolist()

                columns = [
                    {"name": "Batch", "id": "Batch"},
                    {"name": curr_id, "id": curr_id}
                ]
                table_cols = [
                    df[["Batch", prop_id]].to_dict("records")
                ]
                ...
                if len(bar_y) != 0:
                    bar_y.clear()
                    bar_y.extend(batch_y)
                else:
                    bar_y.extend(batch_y)

                ucl = state_dict[curr_id]["ucl"]
                lcl = state_dict[curr_id]["lcl"]
                usl = state_dict[curr_id]["usl"]
                lsl = state_dict[curr_id]["lsl"]
                ...
                if len(bar_histo_trace.keys()) != 0:
                    bar_histo_trace.clear()


                bar_histo_trace["x"] = bar_x[:total_count]
                bar_histo_trace["y"] = bar_y[:total_count]
                bar_histo_trace["type"] = "histogram"
                bar_histo_trace["orientation"] = "v"
                bar_histo_trace["name"] = curr_id + "- histogram"
                bar_histo_trace["xaxis"] = "x"
                bar_histo_trace["yaxis"] = "y"
                bar_histo_trace["marker"] = {"color" : "#f4d44d"}

                return generate_graph(interval, data, curr_id)
            else:

                batch_y = state_dict["Batch"]["data"].tolist()
                curr_y = state_dict[prop_id]["data"].tolist()

                columns = [
                    {"name": "Batch", "id": "Batch"},
                    {"name": prop_id, "id": prop_id}
                ]
                table_cols = [
                    df[["Batch", prop_id]].to_dict("records")
                ]
